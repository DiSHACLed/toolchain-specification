...
2. **Validate the Pipeline Definition**
    ...
    - PipelineShapeValidators validate that the plannd flow of data between processors does not validate any assumptions imposed by Pipeline Components.
<br><br>
3. **Enrich the Pipeline.graph with the Pipeline Build**
    ...
    - Compile ConfigInstances based on the Config of the Steps that each ProcessorInstance implements. The exact format and logic of ConfigInstances are framework-specific and hence this compilation of ConfigInstances has to be hard-coded per framework.
<br><br>
...
<br><br>
5. **Return results**
    - Return the enriched Pipeline.graph. It provides information on: The pipeline that was attempted to be build (Pipeline Definition), the result of that attempt (Pipeline Build) and the resources that were used for that (PipelineComponents).
...
- All predicates and objects stemming from the root IRI are followed recursively. The extracted triples are called "subgraph". It is possible to define which predicates not to follow. and whether inverse relationships should be followed.
...
<br><br>
If we allow Config in different formats, we should also allow Config schemas to be written in different formats. For example, a JSON-schema could validate JSON and yaml-Configs. In this case, the Config Schema cannot be expressed as NodeShape, but it can still be attached to osw:UsesAndAssumptions. A PipelineGenerator should then support validation by JSON-schemas and SHACL-shapes. See [LinkML](https://linkml.io/linkml/intro/overview.html) for related work. 
<br><br>

### 3.2.3. DataShape Validation
DataShape Validation has to evaluate whether an input shape of a Processor match eswith the output shape of the Processor preceeding it. This is a complex problem and to my knowledge, not much research has been done. It requires to evaluate whether two given SHACL-shapes are mutually exclusive or not.
<br><br>
...
<br><br>
In my brief desk research, I stumbled upon [a java-package](https://github.com/paolo7/shacl2fol), which converts SHACL into a "first order logic"- language. I am not sure whether this allows you to check whether the corresponding statements are mutually exclusive, but this seems like a promising starting point. 
<br><br>

### 3.2.4. Dockerfiles
Dockerfiles of Environments are compiled based on the Dockerfiles attached to each PipelineComponent via "osw:hasInstallationInstructions". The compiled Dockerfile should be a [common denominator](https://en.wikipedia.org/wiki/Lowest_common_denominator#Colloquial_usage) of all PipelineComponents included in the Environment. 
<br><br>
Dockerfiles of each Pipeline Component should be self-sufficient, meaning that a Pipeline Component should be instantiable based on its Dockerfile alone. An exception are dependencies on runners, which should be expressed via osw:hasDependency instead. However, given that a common "merged" Dockerfile is compiled for the Pipeline Build, a PipelineGenerator should be tolerant to redundancy across Dockerfiles. 
<br><br>
...
<br><br>
If a Pipeline Build is hosted on more than one Enviroment, not only the Dockerfiles, but also a Docker Compose file needs to be compiled. This Docker Compose file needs to point to each compiled Dockerfile of each Environment. 
<br><br>
...
## 3.3. Frontend
Akin to the [model-view-controller pattern](https://en.wikipedia.org/wiki/Model%E2%80%93view%E2%80%93controller), a frontend should provide a user interface that provides a visual representation of Domain Objects and should allow interacting with them. 
